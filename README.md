...
📦 데이터 소스(공공데이터 포털 API(주차장/상업시설/출국장/항공편/환율))  
↓  
🚌 수집 (Event Hubs)  
↓  
...
- **실시간 파이프라인 구축(2주 내 E2E 가동)**  
  1)Event Hubs → Stream Analytics(SQL Window/집계) → Cosmos DB → Power BI
  - 전처리(형 변환/필드 정규화), 스키마 매핑, 통합 컨테이너 적재 구조 설계
  
  2)ETL(배치/실시간) 파이프라인 설계·구축·운영 및 품질 검증(전/후 건수 확인)
  
  3)원천 데이터 수집·가공 및 품질 기준 수립/검수
...
- **Power BI 자동화**  
  ...
  2)REST API 기반 새로고침 + Teams Webhook 알림 자동화
